...
import { useDocumentTitle } from "../utils/TitleHook";
import {
  Search,
  X,
  Award,
...
const ProgramResults = () => {
  ...
  useDocumentTitle("Program Results - Mafeeh");

  // Flatten placements for search
  ...
  const getStudentBadgeStyle = (position) => {
    const gradients = {
      ...
      default: "bg-gradient-to-br from-gray-50 to-gray-100 shadow-[0_4px_8px_rgba(156,163,175,0.1)]",
    };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-slate-500 max-w-2xl mx-auto">
            Browse competition results and celebrate student achievements across all programs
          </p>
        </div>
        ...
        {Object.keys(groupedByProgram).length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {Object.keys(groupedByProgram).map((programName) => {
              ...
              const program = results.find((p) => p.programName === programName); // ← Get full program
              ...
              return (
                ...
                >
                  ...
                  <div className=" space-x-1 divide-x-2  text-center text-sm font-thin text-slate-500 space-y-1">
                    <strong className="px-0.5">{program?.programId || "N/A"}</strong>
                    <strong className="px-0.5">{program?.category || "Unknown"}</strong>
                    <strong className="px-0.5">{new Date(program?.datePublished).toLocaleDateString()}</strong>
                  ...
                  <div className="flex justify-center items-end gap-4 h-40">
                    {second && (
                      <div className="flex flex-col items-center">
                        <div
                          className={`w-16 h-16 flex items-center justify-center rounded-full mb-2 ${getStudentBadgeStyle(2)}`}
                        >
                          <span className="font-medium text-blue-700">{second.studentId}</span>
                        </div>
                        <div className="px-3 py-1 bg-blue-50 rounded-full flex items-center">
                          {getPositionIcon(2)}
                          <span className="text-xs font-medium text-blue-600 ml-1">2nd</span>
                        </div>
                      </div>
                    )}
                    ...
                    {first && (
                      <div className="flex flex-col items-center">
                        <div
                          className={`w-20 h-20 flex items-center justify-center rounded-full mb-2 ${getStudentBadgeStyle(1)}`}
                        >
                          <span className="font-medium text-amber-700">{first.studentId}</span>
                        </div>
                        <div className="px-3 py-1 bg-amber-50 rounded-full flex items-center">
                          {getPositionIcon(1)}
                          <span className="text-xs font-bold text-amber-600 ml-1">1st</span>
                        </div>
                      </div>
                    )}
                    ...
                    {third && (
                      <div className="flex flex-col items-center">
                        <div
                          className={`w-16 h-16 flex items-center justify-center rounded-full mb-2 ${getStudentBadgeStyle(3)}`}
                        >
                          <span className="font-medium text-rose-700">{third.studentId}</span>
                        </div>
                        <div className="px-3 py-1 bg-rose-50 rounded-full flex items-center">
                          {getPositionIcon(3)}
                          <span className="text-xs font-medium text-rose-600 ml-1">3rd</span>
                        </div>
                      </div>
                    )}
                  </div>
          ...
          <div className="text-center py-16 bg-white rounded-2xl shadow-xs border border-slate-100/70">
        ...
        {/* Modal - No animation on close */}
        {selectedProgram && (
          <>
            <div className="fixed inset-0 bg-black bg-opacity-30 z-40" onClick={() => setSelectedProgram(null)}></div>
              ...
              <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto relative">
                ...
                <div className="p-6 md:p-8">
                  ...
                  {(() => {
                    ...
                    const { first, second, third } = podiumLayout(placements);
                    const others = placements.filter((p) => p.position > 3 || !p.position);
                      ...
                      <>
                        ...
                        <div className="flex justify-center items-end gap-6 mb-10 h-48">
                          {second && (
                            <div className="flex flex-col items-center">
                              <div
                                className={`w-20 h-20 flex items-center justify-center rounded-full mb-3 ${getStudentBadgeStyle(2)}`}
                              >
                                <span className="font-medium text-blue-700 text-lg">{second.studentId}</span>
                              </div>
                              <div className="px-4 py-1.5 bg-blue-50 rounded-full shadow-inner flex items-center">
                                {getPositionIcon(2)}
                                <span className="text-sm font-medium text-blue-600 ml-1.5">Second Place</span>
                              </div>
                              <div className="mt-2 px-3 py-1 bg-white border border-blue-100 rounded-full">
                                <span className="text-xs text-blue-500">Grade: {second.grade || "-"}</span>
                              </div>
                            </div>
                          )}
                          ...
                          {first && (
                            <div className="flex flex-col items-center">
                              <div
                                className={`w-24 h-24 flex items-center justify-center rounded-full mb-3 ${getStudentBadgeStyle(1)}`}
                              >
                                <span className="font-medium text-amber-700 text-xl">{first.studentId}</span>
                              </div>
                              <div className="px-4 py-1.5 bg-amber-50 rounded-full shadow-inner flex items-center">
                                {getPositionIcon(1)}
                                <span className="text-sm font-bold text-amber-600 ml-1.5">First Place</span>
                              </div>
                              <div className="mt-2 px-3 py-1 bg-white border border-amber-100 rounded-full">
                                <span className="text-xs text-amber-500">Grade: {first.grade || "-"}</span>
                              </div>
                            </div>
                          )}
                          ...
                          {third && (
                            <div className="flex flex-col items-center">
                              <div
                                className={`w-20 h-20 flex items-center justify-center rounded-full mb-3 ${getStudentBadgeStyle(3)}`}
                              >
                                <span className="font-medium text-rose-700 text-lg">{third.studentId}</span>
                              </div>
                              <div className="px-4 py-1.5 bg-rose-50 rounded-full shadow-inner flex items-center">
                                {getPositionIcon(3)}
                                <span className="text-sm font-medium text-rose-600 ml-1.5">Third Place</span>
                              </div>
                              <div className="mt-2 px-3 py-1 bg-white border border-rose-100 rounded-full">
                                <span className="text-xs text-rose-500">Grade: {third.grade || "-"}</span>
                              </div>
                            </div>
                          )}
                        </div>
                          ...
                          {others.length > 0 ? (
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                              {others.map((p, i) => (
                                ...
                                >
                                  ...
                                  >
                                    <span className="font-medium text-slate-700 text-sm">{p.studentId}</span>
                                  </div>
                            ...
                            <div className="text-center py-6">
                              <div className="inline-block px-4 py-2 bg-slate-50 rounded-full border border-slate-200">
                                <span className="text-slate-400">No other participants</span>
                              </div>
                            </div>
                          )}
                        </div>
